...
export default function FarmerChats() {
  ...
  const initializeChats = async () => {
    try {
      if (!user) {
        console.log('FarmerChats: User not available, cannot initialize chats.');
        return;
      }
      console.log('FarmerChats: Initializing chats for user ID:', user.id);

      // Initialize Firebase auth
      const authSuccess = await firebaseChatService.initializeAuth();
      if (!authSuccess) {
        console.error('FarmerChats: Failed to initialize Firebase auth');
        setLoading(false);
        return;
      }
      ...
      const unsubscribe = firebaseChatService.subscribeToChats(user.id, (userChats) => {
        console.log('FarmerChats: Received user chats:', userChats);
        setChats(userChats);
        setLoading(false);
      });
  ...
  const handleChatPress = (chat: Chat) => {
    router.push({
      ...
      params: {
        ...
        labourId: chat.labourId, // Pass the labourId from the chat object
        labourName: chat.labourName, // Pass the labourName from the chat object
        ...
        {loading ? (
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtext}>
              Chats will appear here when labourers message you about jobs
            </Text>
          </View>
        ) : (
          chats.map((chat) => (
            ...
            >
              <View style={styles.avatarContainer}>
                <Text style={styles.avatarText}>
                  {getInitials(chat.labourName)}
                </Text>
              </View>
              
              <View style={styles.chatContent}>
                <View style={styles.chatHeader}>
                  <Text style={styles.chatName}>{chat.labourName}</Text>
